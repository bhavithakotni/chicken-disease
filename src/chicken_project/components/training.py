...
from src.chicken_project.entity.config_entity import TrainingConfig
from pathlib import Path
class Training:
    def __init__(self,config: TrainingConfig):
        self.config= config
    def get_basemodel(self):
        self.model= tf.keras.models.load_model(self.config.updated_basemodel,compile=False)
        self.model.compile(optimizer=tf.keras.optimizers.SGD(learning_rate=self.config.params_learning_rate),loss=tf.keras.losses.CategoricalCrossentropy(),metrics=["accuracy"])
    ...
    def train(self,callback_list: list):
        ...
        self.validation_steps= self.valid_genetator.samples//self.valid_genetator.batch_size
        self.model.fit(self.train_generator,
                       epochs= self.config.params_epochs,
                       steps_per_epoch=self.steps_per_epoch,
                       validation_steps=self.validation_steps,
                       validation_data=self.valid_genetator,
